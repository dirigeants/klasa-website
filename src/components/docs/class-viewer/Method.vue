				...
				<div class="level-left">
					<div class="level-item has-text-left">
						<span v-if="method.access === 'private'">
							<span class="tag is-warning" title="This method is private, and may change or be removed at any time.">
								Private
							</span>&nbsp;
						</span>
						<span v-if="method.scope === 'static'">
							<span class="tag is-primary" title="This method is on the class constructor function, not instances.">
								Static
							</span>&nbsp;
						</span>
						<span v-if="method.async">
							<span class="tag is-success" title="This method is async, and returns a promise.">
								Async
							</span>&nbsp;
						</span>
						...
						<span v-if="method.deprecated">
							<span class="tag is-danger" title="This method is deprecated, and may be removed in a future version.">
								Deprecated
							</span>&nbsp;
						</span>
						<span v-if="method.generator">
							<span class="tag is-dark" title="This method returns a generator function.">
								Generator
							</span>&nbsp;
						</span>
					</div>
		...
		<div class="card-content">
			...
			<div v-if="emits">
				...
				<ul v-if="emits.length > 1">
					<li v-for="(event, key) in emits" :key="key">
						<router-link :to="event.link">
							{{ event.text }}
						</router-link>
					</li>
				</ul>
				<router-link v-else :to="emits[0].link">
					{{ emits[0].text }}
				</router-link>
			</div>
			...
			<div v-if="method.examples">
				...
				<pre v-for="(example, key) in method.examples" :key="key" v-highlightjs><code class="javascript">{{ example }}</code></pre>
			</div>
		</div>
		<footer class="card-footer">
			<p v-if="method.yields" class="card-footer-item">
				<span>
					...
					<span v-if="method.yields">
						<span v-for="(rtrn, index) of method.yields.types || method.yields" :key="index">
							<types :names="rtrn" :variable="method.yields.variable" :nullable="method.yields.nullable" :docs="docs" />
							<span v-if="index < (method.yields.types || method.yields).length - 1">
								|
							</span>
						</span>
					</span>
			...
			<p v-if="!method.yields || method.returns" class="card-footer-item">
				<span>
					...
					<span v-if="method.returns">
						<span v-for="(rtrn, index) of method.returns.types || method.returns" :key="index">
							<types :names="rtrn" :variable="method.returns.variable" :nullable="method.returns.nullable" :docs="docs" />
							<span v-if="index < (method.returns.types || method.returns).length - 1">
								|
							</span>
						</span>
					</span>
			...
			<p v-if="method.throws" class="card-footer-item">
				<span>
					...
					<span v-for="(thrw, index) of method.throws" :key="index">
						<types :names="thrw" :docs="docs" />
						<span v-if="index < method.throws.length - 1">
							|
						</span>
					</span>
				</span>
